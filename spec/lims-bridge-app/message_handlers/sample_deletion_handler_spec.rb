...
module Lims::BridgeApp::MessageHandlers
  describe SampleDeletionHandler do
    ...
    context "with a valid call" do
      context "with one sample" do
        ...
        it "calls the delete_sample method" do
          sequencescape.should_receive(:delete_sample).with(sample_uuid).and_call_original 
      ...
      context "with an array of samples" do
        ...
        it "calls the methods to delete sample" do
          samples.each do |sample_data|
            bus.should_receive(:publish).with(sample_data[:uuid])
            sequencescape.should_receive(:delete_sample).with(sample_data[:uuid]).and_call_original 
          end
          metadata.should_receive(:ack)
        end
      end
